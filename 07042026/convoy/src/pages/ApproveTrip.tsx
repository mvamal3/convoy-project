...
import { approveTripAPI } from "@/contexts/PostApi";
import { getRunningConveyDetails } from "@/contexts/GetApi";
...
const ApproveTrip = () => {
  ...
  const handlesapprovetrip = async (tripId, buttonId) => {
    try {
      // Fetch details about the running convey
      const res = await getRunningConveyDetails(accessToken, user.checkpostid);
      // Normalize the response to get convey data
      const running = res?.data?.data ?? res?.data ?? res;
      const conveyId = running?.conveyid;

      // Handle when no convey is active
      if (!conveyId) {
        toast({
          title: "No running convey found",
          description: "Cannot approve trip without an active convey.",
          variant: "destructive",
        });
        return;
      }
  ...
  // Commented out for reject with remarks
  // const handleCancelTrip = async (tripId, buttonId) => {
  //   try {
  //     const res = await getRunningConveyDetails(accessToken, user.checkpostid);
  //     // Normalize the response to get convey data
  ...
  //       toast({
  //         title: "No running convey found",
  //         description: "Cannot Ca trip without an active convey.",
  //         variant: "destructive",
  //       });
  //       return;
  //     }
  ...
  const handleRejectTrip = async () => {
    ...
    try {
      ...
      if (!conveyId) {
        toast({
          title: "No running convey found",
          description: "Cannot reject trip without an active convey.",
          variant: "destructive",
        });
        return;
      }
      ...
      const payload = {
        ...
        checkpost_id: user.checkpostid,
        approveby: user.pol_reg_id,
        remarks: rejectRemarks.trim(), // ✅ USER INPUT ONLY
        astatus: 0,
        buttonId: 3,
      };
    ...
    <DashboardLayout>
      <div className="space-y-10 pb-8 px-3 sm:px-6 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-4 sm:gap-6 mt-8">
          ...
          >
            <CheckCircle className="w-6 h-6" /> Approve Trip
          </Button>
