...
const PoliceManageTrip = () => {
  ...
  const handleStartConvoy = async () => {
    if (!selectedConveyId) {
      toast({
        title: "Error",
        description: "Please select a convey",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleStopConvey = async () => {
    ...
    try {
      ...
      if (response.success) {
        toast({
          title: "Success",
          description: response.message || "Convey stopped successfully.",
        });
        await fetchRunningConvey();
        fetchTripList();
        ...
        toast({
          title: "Failed",
          description: response.message || "Unable to stop convey.",
          variant: "destructive",
        });
      }
    } catch (error) {
      toast({
        title: "Error",
        description:
          error.message || "Something went wrong while stopping convey.",
        variant: "destructive",
      });
    }
  };
          ...
          <div className="flex flex-col">
            ...
            {runningConveyId ? (
              <p className="text-sm text-green-600 mt-1">
                Running Convey: {runningConvey?.convey_name ?? "Unknown"} <br />
